    ...
    return (
        ...
        <View>
            ...
            >
                ...
                {
                    articleParagraphs.map((lines, index) => {
                        // console.log('INDEX',index)
                        return(
                            <View key={index}>
                                {index % 3 === 0 && <Image style={styles.image} source={{uri: getImage(index)}}/>}
                                <Text style={styles.paragraph}>{lines}</Text>
                            </View>
                        );
                    })
                        ...
                        articleParagraphs.slice(0, 3).map((text, index) => {
                            return(
                                <View key={`featured-${index}`} style={{flexDirection: 'row', marginBottom: 10}}>
                                    ...
                                    <Text style={styles.paragraph} numberOfLines={3}>{text}</Text>
                                    {/* {console.log('Key:',key)} */}
                                </View>
                            );
                        })
            ...
            {bottomActions && (
            <Animated.View 
              style={[styles.bottomActions, {position: 'absolute', right: 0, left: 0, bottom: 0, paddingHorizontal: 20, 
              transform: [{
            ...
            >
                ...
                <View style={{ flexDirection: 'row' }}>
                    <Animated.View style={[styles.icon, {
                        opacity: scrollY.interpolate({inputRange, outputRange: [0, 0, 0, 0, 1]})
                    }]}>
